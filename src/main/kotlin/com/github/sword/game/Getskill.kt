...
object Getskill {
    ...
    fun e(e: PlayerCastSkillEvent.Pre) {
        ...
        for (i in config.getStringList("skill-black").indices) {
            if (world == config.getStringList("skill-black")[i]){
                ...
                if (sound != null) {
                    player.playSound(player.location, org.bukkit.Sound.valueOf(sound.replace('.', '_').uppercase()), 1.0f, 1.0f)
                }
            }
        }
    }
    ...
    fun k(e: GermKeyDownEvent) {
        ...
        if (key == KeyType.KEY_R && !sk) {
            player.sendTitle(config.getString("skill-mode-title1")?.let { parse(it) }, config.getString("skill-mode-subtitle1")?.let { parse(it) }, 10, 20 ,10)
            sk = true
        } else if (key == KeyType.KEY_R) {
            player.sendTitle(config.getString("skill-mode-title2")?.let { parse(it) }, config.getString("skill-mode-subtitle2")?.let { parse(it) }, 10, 20 ,10)
            sk = false
        }
    }
}